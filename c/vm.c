...
#define E(name) set_global(vm, "C_" #name, MAKE_INT(name))
	...
	E(PCRE_ANCHORED); E(PCRE_AUTO_CALLOUT); E(PCRE_BSR_ANYCRLF); E(PCRE_BSR_UNICODE); E(PCRE_CASELESS);
	E(PCRE_CONFIG_BSR); E(PCRE_CONFIG_JIT); E(PCRE_CONFIG_JITTARGET); E(PCRE_CONFIG_LINK_SIZE); E(PCRE_CONFIG_MATCH_LIMIT);
	E(PCRE_CONFIG_MATCH_LIMIT_RECURSION); E(PCRE_CONFIG_NEWLINE); E(PCRE_CONFIG_PARENS_LIMIT); E(PCRE_CONFIG_POSIX_MALLOC_THRESHOLD); E(PCRE_CONFIG_STACKRECURSE);
	E(PCRE_CONFIG_UNICODE_PROPERTIES); E(PCRE_CONFIG_UTF16); E(PCRE_CONFIG_UTF32); E(PCRE_DATE); E(PCRE_DFA_RESTART);
	E(PCRE_DFA_SHORTEST); E(PCRE_DOLLAR_ENDONLY); E(PCRE_DOTALL); E(PCRE_DUPNAMES); E(PCRE_ERROR_BADCOUNT);
	E(PCRE_ERROR_BADENDIANNESS); E(PCRE_ERROR_BADLENGTH); E(PCRE_ERROR_BADMAGIC); E(PCRE_ERROR_BADMODE); E(PCRE_ERROR_BADNEWLINE);
	E(PCRE_ERROR_BADOFFSET); E(PCRE_ERROR_BADOPTION); E(PCRE_ERROR_BADPARTIAL); E(PCRE_ERROR_BADUTF16); E(PCRE_ERROR_BADUTF16_OFFSET);
	E(PCRE_ERROR_BADUTF32); E(PCRE_ERROR_BADUTF8); E(PCRE_ERROR_BADUTF8_OFFSET); E(PCRE_ERROR_CALLOUT); E(PCRE_ERROR_DFA_BADRESTART);
	E(PCRE_ERROR_DFA_RECURSE); E(PCRE_ERROR_DFA_UCOND); E(PCRE_ERROR_DFA_UITEM); E(PCRE_ERROR_DFA_UMLIMIT); E(PCRE_ERROR_DFA_WSSIZE);
	E(PCRE_ERROR_INTERNAL); E(PCRE_ERROR_JIT_BADOPTION); E(PCRE_ERROR_JIT_STACKLIMIT); E(PCRE_ERROR_MATCHLIMIT); E(PCRE_ERROR_NOMATCH);
	E(PCRE_ERROR_NOMEMORY); E(PCRE_ERROR_NOSUBSTRING); E(PCRE_ERROR_NULL); E(PCRE_ERROR_NULLWSLIMIT); E(PCRE_ERROR_PARTIAL);
	E(PCRE_ERROR_RECURSELOOP); E(PCRE_ERROR_RECURSIONLIMIT); E(PCRE_ERROR_SHORTUTF16); E(PCRE_ERROR_SHORTUTF8); E(PCRE_ERROR_UNKNOWN_NODE);
	E(PCRE_ERROR_UNKNOWN_OPCODE); E(PCRE_ERROR_UNSET); E(PCRE_EXTENDED); E(PCRE_EXTRA); E(PCRE_EXTRA_CALLOUT_DATA);
	E(PCRE_EXTRA_EXECUTABLE_JIT); E(PCRE_EXTRA_MARK); E(PCRE_EXTRA_MATCH_LIMIT); E(PCRE_EXTRA_MATCH_LIMIT_RECURSION); E(PCRE_EXTRA_STUDY_DATA);
	E(PCRE_EXTRA_TABLES); E(PCRE_FIRSTLINE); E(PCRE_INFO_BACKREFMAX); E(PCRE_INFO_CAPTURECOUNT); E(PCRE_INFO_DEFAULT_TABLES);
	E(PCRE_INFO_FIRSTBYTE); E(PCRE_INFO_FIRSTCHAR); E(PCRE_INFO_FIRSTCHARACTER); E(PCRE_INFO_FIRSTCHARACTERFLAGS); E(PCRE_INFO_FIRSTTABLE);
	E(PCRE_INFO_HASCRORLF); E(PCRE_INFO_JCHANGED); E(PCRE_INFO_JIT); E(PCRE_INFO_JITSIZE); E(PCRE_INFO_LASTLITERAL);
	E(PCRE_INFO_MATCH_EMPTY); E(PCRE_INFO_MATCHLIMIT); E(PCRE_INFO_MAXLOOKBEHIND); E(PCRE_INFO_MINLENGTH); E(PCRE_INFO_NAMECOUNT);
	E(PCRE_INFO_NAMEENTRYSIZE); E(PCRE_INFO_NAMETABLE); E(PCRE_INFO_OKPARTIAL); E(PCRE_INFO_RECURSIONLIMIT); E(PCRE_INFO_REQUIREDCHAR);
	E(PCRE_INFO_REQUIREDCHARFLAGS); E(PCRE_INFO_SIZE); E(PCRE_INFO_STUDYSIZE); E(PCRE_JAVASCRIPT_COMPAT); E(PCRE_MAJOR);
	E(PCRE_MINOR); E(PCRE_MULTILINE); E(PCRE_NEVER_UTF); E(PCRE_NEWLINE_ANY); E(PCRE_NEWLINE_ANYCRLF);
	E(PCRE_NEWLINE_CR); E(PCRE_NEWLINE_CRLF); E(PCRE_NEWLINE_LF); E(PCRE_NO_AUTO_CAPTURE); E(PCRE_NO_AUTO_POSSESS);
	E(PCRE_NO_START_OPTIMISE); E(PCRE_NO_START_OPTIMIZE); E(PCRE_NOTBOL); E(PCRE_NOTEMPTY); E(PCRE_NOTEMPTY_ATSTART);
	E(PCRE_NOTEOL); E(PCRE_NO_UTF16_CHECK); E(PCRE_NO_UTF32_CHECK); E(PCRE_NO_UTF8_CHECK); E(PCRE_PARTIAL);
	E(PCRE_PARTIAL_HARD); E(PCRE_PARTIAL_SOFT); E(PCRE_STUDY_EXTRA_NEEDED); E(PCRE_STUDY_JIT_COMPILE); E(PCRE_STUDY_JIT_PARTIAL_HARD_COMPILE);
	E(PCRE_STUDY_JIT_PARTIAL_SOFT_COMPILE); E(PCRE_UCP); E(PCRE_UNGREEDY); E(PCRE_UTF16); E(PCRE_UTF16_ERR1);
	E(PCRE_UTF16_ERR2); E(PCRE_UTF16_ERR3); E(PCRE_UTF16_ERR4); E(PCRE_UTF32); E(PCRE_UTF32_ERR1);
	E(PCRE_UTF32_ERR2); E(PCRE_UTF32_ERR3); E(PCRE_UTF8); E(PCRE_UTF8_ERR1); E(PCRE_UTF8_ERR10);
	E(PCRE_UTF8_ERR11); E(PCRE_UTF8_ERR12); E(PCRE_UTF8_ERR13); E(PCRE_UTF8_ERR14); E(PCRE_UTF8_ERR15);
	E(PCRE_UTF8_ERR16); E(PCRE_UTF8_ERR17); E(PCRE_UTF8_ERR18); E(PCRE_UTF8_ERR19); E(PCRE_UTF8_ERR2);
	E(PCRE_UTF8_ERR20); E(PCRE_UTF8_ERR21); E(PCRE_UTF8_ERR22); E(PCRE_UTF8_ERR3); E(PCRE_UTF8_ERR4);
	E(PCRE_UTF8_ERR5); E(PCRE_UTF8_ERR6); E(PCRE_UTF8_ERR7); E(PCRE_UTF8_ERR8); E(PCRE_UTF8_ERR9);
