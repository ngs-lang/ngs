...
" builtin functions
syn keyword ngsFunction c_close c_execve c_exit c_fork c_lseek copy c_open c_read c_waitpid C_WEXITSTATUS C_WTERMSIG compile del dump echo get get_c_errno globals hash impl_not_found_hook in inherit is keys len load not decode_json push shift values
...
" stdlib functions
syn keyword ngsFunction acquire all any assert close_reading_end close_writing_end count dup2 dup2_reading_end dup2_writing_end each each_idx_key_val each_idx_val error expose debug die filter find_in_path first flatten group has identity in index join len limit log map max min next none partial partial_tail pmap pos ptimes read release reverse split status Strs uniq update wait without write zip
