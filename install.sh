...
if [[ "$OS" == "Darwin" ]]; then
	...
	brew install libgc libffi peg cmake pandoc awk make pkg-config json-c pcre2 gnu-sed
else
	# Presumably Linux
	if type yum &>/dev/null;then
		...
		$SUDO pacman -Sy --noconfirm peg make cmake pandoc pkgconfig pcre2
	elif type apt &>/dev/null;then
		echo "  + On Linux / apt. Installing apt packages."
		$SUDO apt-get install -y libgc-dev libffi-dev libjson-c-dev peg libpcre2-dev make cmake pandoc pkg-config build-essential
